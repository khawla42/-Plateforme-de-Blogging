...
const Loader = () => {
  ...
  useEffect(() => {
    // Vérifier si le loader a déjà été vu
    const hasSeenLoader = localStorage.getItem('hasSeenLoader');

    if (hasSeenLoader) {
      // Si oui, directement rediriger vers la page principale
      navigate('/home');
    } else {
      // Si non, afficher le loader pendant 5 secondes et ensuite rediriger
      const timer = setTimeout(() => {
        setLoading(false);
        localStorage.setItem('hasSeenLoader', 'true'); // Marquer que le loader a été vu
        navigate('/home');
